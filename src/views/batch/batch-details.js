  ...
  useEffect(() => {
    async function fetchDocumentData() {
      if (data && data.documents.length > 0) {
        let isPatientSplitDocument = data.documents.filter(
          (x) => x.is_patient_split
        );
            ...
            <Card className="card-user">
              <CardBody className="document-list-item-height">
                <ListGroup>
                  {data &&
                    data.documents
                      .filter((x) => x.is_patient_split)
                      .map((document, i) => (
                        <ListGroupItem
                          action
                          active={
                            document.id === selectedDocument?.id ? true : false
                          }
                          color="info"
                          tag="button"
                          onClick={() => onDocumentChangeHandler(document)}
                        >
            ...
            {selectedDocument && (
              <Card className="card-user">
                ...
                <CardBody className="document-viewer">
                  <MemoizedViewer
                    key={`document-pdf-viewer-${selectedDocument.id}`}
                    fileUrl={getDocumentDownloadUrl(id, selectedDocument.id)}
                  />
                </CardBody>
              </Card>
            )}
            {selectedDocument &&
              selectedDocument.parent_document_id !== null && (
                <Card className="card-user">
                  <CardHeader>
                    <CardText>
                  ...
                  <CardBody className="document-viewer">
                    <MemoizedViewer
                      key={`document-pdf-viewer-${selectedDocument.parent_document_id}`}
                      fileUrl={getDocumentDownloadUrl(
                        id,
                        selectedDocument.parent_document_id
                      )}
                    />
                  </CardBody>
                </Card>
                    ...
                    <Col>
                      <Card color="" className="mb-2">
                        ...
                        <CardBody className="pt-2">
                          <Nav className="nav-pills-info category-tab" pills>
                            <NavItem>
                              <NavLink
                                ...
                                color="primary"
                                className={
                                  horizontalTabs[index] === `document`
                                    ? "active btn-sm"
                                    : "btn-sm"
                                }
                                onClick={() =>
                                  changeActiveTab("document", index)
                                }
                              >
                                Document
                              </NavLink>
                            ...
                            <NavItem>
                              <NavLink
                                ...
                                href="#pablo"
                                className={
                                  horizontalTabs[index] === "summary"
                                    ? "active"
                                    : ""
                                }
                                onClick={() =>
                                  changeActiveTab("summary", index)
                                }
                              >
                                Summary
                              </NavLink>
                          ...
                          <TabContent
                            className="tab-space pt-0 pb-0"
                            activeTab={horizontalTabs[index]}
                          >
                            <TabPane tabId="document">
                              <Card color="light" className="my-2">
                                <CardBody className="p-2 tab-content-height">
                                  ...
                                  <MemoizedViewer
                                    key={`pdf-viewer-${index}`}
                                    fileUrl={getSplitDocumentDownloadUrl(
                                      id,
                                      selectedDocument.id,
                                      detail.id
                                    )}
                                  ...
                                  <Card className="m-0">
                                    <CardBody className="p-0 tab-content-height">
                                      <Table>
                                        ...
                                        <tbody>
                                          {Object.keys(detail.fields).length >
                                          0 ? (
                                            <>
                                              {Object.entries(
                                                detail.fields
